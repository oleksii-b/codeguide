...
Весь код має бути написаний у нижньому регістрі. Це відноситься до назв елементів, назв атрибутів, значень атрибутів (окрім тексту/**CDATA**), селекторів, властивостей та їх значнь (окрім тексту).
...
Відокремлення структури від її вигляду та поведінки допомогає забезпечити підтримку коду. Зміна шаблонів та HTML-документів завжди займає більше часу, ніж зміна файлів стилів та скриптів.
...
Не прописуйте назву протоколу (**http:**, **https:**) у посиланнях на зображення або інші медіа-ресурси, файли стилів або скрипти, якщо ці ресурси є доступними за обома протоколами.
...
### Посилання-мнемоніки
#### Уникайте використання посиланнь-мнемонік

Нема сенсу використовувати такі посилання мнемоніки як `&mdash;`, `&rdquo;`, або `&#x263a;`, коли всі команди в файлах та редакторах викристовують єдине кодування (**UTF-8**).

Виключенням з цього правила є лише службові символи HTML (наприкад `<` та `&`), а також допоміжні символи, що не мають відображення (наприклад, нерозривний пробіл).
...
Вказувати атрибут `type` в даному випадку не обов'язково, оскільки HTML5 використовує `text/css` та `text/javascript` за замовчуванням. Це працюватиме навіть в старих браузерах.
...
Уникайте використання зашифрованих імен класів, або таких, що описують зовнішній вигляд чи поведінку елемента. Натомість намагайтеся відобразити в імені классу сутність елементу, до якого він додається, або використати шаблонне ім'я.
...
В CSS існує багато скорочених форм записів властивостей (font, border тощо). Використовуйте скорочені властивості всюду, де це можливо, навіть якщо передається лише одне із можливих значень.
...
Не вказуйте одиніці виміру для нульових значень, оскільки це не є обов'язковим і скоротить код.
...
#### Використовуйте скорочений шістнадцятиричний запис кольорів там, де це є можливим
...
Завжди ставте відступи для будь-якого вмісту блоків (наприклад, для правил всередині правил або декларувань), а також закриваючу дужку, щоб відобразити ієрархію та полегшити розуміння коду.
...
```
#### Відокремлюйте селектори та декларування переносом рядку
...
#### Відокремлюйте правила переносом рядку
